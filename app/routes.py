...
@router.post("/api/login", response_model=schemas.Token)
async def login(form_data: schemas.Login, db: AsyncSession = Depends(get_db_session)):
    result = await db.execute(models.User.__table__.select().where(models.User.email == form_data.email))
    user = result.first()
...
@router.get("/api/profile", response_model=schemas.User)
async def profile(current_user: models.User = Depends(auth.get_current_user)):
    return current_user